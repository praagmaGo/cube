...
func initGc(w, h int) (image.Image, *draw2dimg.GraphicContext) {
...
func DrawPlan(gc draw2dimg.GraphicContext,num uint8){
...
//  saveToPngFile("cube"+strconv.Itoa(ref), i)
  saveToPngFile(fmt.Sprintf("cube%04d",ref),i)
}
...
//  gc.Rect(0, 0, w-1, w-1)
  ...
  gc.LineTo(a*math.Cos(angle),a*math.Sin(angle))
  gc.LineTo(0                ,2*a*math.Sin(angle))
